...
follow-up commit. **Open: C9.4, C15.4, C16.5, C18, C19.3–C19.4/C19.6–C19.7/C19.22–C19.23, C21.4–C21.5,
and the new C23 PostgreSQL execution-backend track.**
...
- [ ] **C19.4 Make memory-service discovery recursive.** `runtime.memory_service_for_document` only scans top-level `document.do`; nested agents in `do`, `try`, `fork`, or `switch` are missed.
...
- [ ] **C23.1 Audit the upstream PostgreSQL backend.** Document the exact schema, tables (`workflow_runs`, `step_attempts`), indexes, namespace/schema isolation, migration strategy, and query patterns used by OpenWorkflow. Cite upstream docs.
- [ ] **C23.2 Design the ADK-aligned PostgreSQL schema.** Map upstream concepts to this translator: run record (status, input, output, state, region), step attempts (task name, attempt number, output, error, started/finished at), retry tracking, and namespace. Decide on JSONB vs normalized columns.
- [ ] **C23.3 Add `PostgresRunHistory`.** Implement an async `RunHistory` backend using `asyncpg` with a connect URL, schema/namespace support, and automatic migrations (idempotent DDL). Keep `InMemoryRunHistory` and `SQLiteRunHistory` for dev/tests.
- [ ] **C23.4 Split step attempts into a separate table.** Currently `run_workflow` stores events inside `workflow_runs.event_log`. Add a `step_attempts` table and record each task execution/retry as a row, supporting upstream-style step history and replay.
- [ ] **C23.5 Add worker database-polling mode.** Extend `WorkflowWorker` (or add a new worker class) to poll `workflow_runs` for pending runs, claim them atomically, execute via `run_workflow`, and update status. Make this an alternative to the broker-driven mode.
- [ ] **C23.6 Add heartbeats and crash recovery.** Workers extend a lease/heartbeat timestamp on claimed runs; orphaned runs become claimable again after the heartbeat expires.
- [ ] **C23.7 Add query helpers for execution stats.** Implement filters by status, workflow name, namespace, time range, and error type; expose counts, p95/p99 duration, and failure rate.
...
- **C19.1–C19.2, C19.5, C19.8–C19.11, C19.20–C19.21** — diagnostic path, recursive state-schema derivation, agent boolean removal, legacy rejection, CLI catalog-mode reconciliation, rejection tests, agent-boolean tests. **C19.10** hardened metadata access in loader validators.
