...
OpenWorkflowDocument (typed pydantic model)
   │  2. validate against vendored 1.0.3 JSON schema + extension schema
   │  3. resolve config (env > per-task agent config > project defaults)
   ▼
Resolved tasks + agent characteristics
   │  4. translate: each task → ADK node (LlmAgent | FunctionNode | nested Workflow)
...
adk.Workflow(edges=…, state_schema=…)
   │  5. run via adk.Runner + InMemorySessionService (dev) / Vertex (prod)
   ▼
events / output
```
...
| **task with agent config** | `adk.Agent`(model, instruction, tools, output_schema, output_key) |
| `use.models` (extension) | named `ModelSpec` registry; agent `model: {use: name}` resolves a model + generation-config bundle |
| `use.providers` (extension) | named `ProviderConfig` registry; a model references one; resolved to an ADK `BaseLlm` via the provider factory |
| `use.memories` (extension) | named `MemoryConfig` registry; agent `memory: {use: name}` resolves to an ADK `BaseMemoryService` |
